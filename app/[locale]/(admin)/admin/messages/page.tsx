...
import { mockClients, messagesForClient } from '@/lib/mock-data';
...
export default function AdminMessagesPage() {
  ...
  // AI agent enabled/disabled per client (Section 4.3).
  const [aiEnabled, setAiEnabled] = useState<Record<string, boolean>>(
    Object.fromEntries(mockClients.map((c) => [c.id, true])),
  );
  const [selected, setSelected] = useState(mockClients[0].id);

  const thread = messagesForClient(selected);
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="block truncate text-xs text-text-secondary">
                        {messagesForClient(c.id).at(-1)?.content ?? '—'}
                      </span>
                    </span>
                  </button>
                </li>
        ...
        <Card className="flex flex-col">
          <div className="flex items-center justify-between border-b border-border p-4">
            <span className="font-medium">
              {mockClients.find((c) => c.id === selected)?.full_name}
            </span>
            <button
